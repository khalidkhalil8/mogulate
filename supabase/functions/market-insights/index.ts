...
async function discoverCompetitors(idea: string): Promise<{ competitors: Competitor[], error?: string }> {
  ...
  if (!PERPLEXITY_API_KEY) {
    ...
    return { competitors: [], error: 'Missing Perplexity API key' };
  }
  ...
  if (!response.ok) {
    console.log(`First Perplexity attempt failed with status ${response.status}, retrying after 500ms...`);
    ...
    if (!response.ok) {
      ...
      console.error('Both Perplexity API attempts failed:', errorData);
      return { competitors: [], error: `Perplexity API Error: ${response.status}` };
    }
  }
...
async function analyzeMarketGaps(idea: string, competitors: Competitor[]): Promise<{ analysis: MarketGapAnalysis | null, error?: string }> {
  ...
  if (!OPENAI_API_KEY) {
    ...
    return { analysis: null, error: 'Missing OpenAI API key' };
  }
...
serve(async (req) => {
  ...
  try {
    console.log('Starting market-insights function');
    
    const requestData = await req.json();
    const { idea, competitors: providedCompetitors } = requestData;

    console.log('Request data:', { idea: idea?.substring(0, 100), hasCompetitors: !!providedCompetitors });
    ...
    // Market gap analysis
    console.log('Performing market gap analysis with scoring');
    ...
    console.error('Unexpected error in market-insights function:', error);
    return new Response(JSON.stringify({
      success: false,
